...
def _build_item(ticker, name, curr, prev):
    if curr is None or prev is None or prev == 0:
        return {"ticker": ticker, "name": name, "price": None, "change": None, "changePct": None, "direction": "flat"}
    change = curr - prev
...
def _fetch_market_data() -> list[dict]:
    ...
    try:
        raw = yf.download(tickers=tickers, period="5d", interval="1d", progress=False, auto_adjust=True)
        close = raw.get("Close")
        if close is None:
